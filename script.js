...
async function loadStream() {
    ...
    if (!channel) return;
    localStorage.setItem('last_twitch_channel', channel);
...
renderFavorites();
fetchGlobal7TV();
const savedChannel = localStorage.getItem('last_twitch_channel');
if (savedChannel) {
...
// ==========================================
// ПЕРЕЗАПУСК ЧАТА ПО КНОПКЕ
// ==========================================
function reconnectChat() {
    const btn = document.getElementById('reconnect-chat-btn');
    if (btn) btn.classList.add('spin-anim');

    const channel = document.getElementById('channelName').value.toLowerCase().replace(/\s+/g, '');

    if (!channel) {
        console.warn("Канал не выбран, перезапуск отменен.");
    ...
    // Выводим системное уведомление в чат
    const chatContainer = document.getElementById('chat-messages');
    if (chatContainer) {
        const sysMsg = document.createElement('div');
        sysMsg.style.color = '#adadb8';
        sysMsg.style.fontStyle = 'italic';
        sysMsg.style.padding = '6px 10px';
        sysMsg.style.textAlign = 'center';
        sysMsg.innerHTML = '⌛ <i>Переподключение к чату...</i>';
        chatContainer.appendChild(sysMsg);
        chatContainer.scrollTop = chatContainer.scrollHeight;
    }

    // Запускаем подключение заново через полсекунды
    setTimeout(() => {
        initChat(channel);
        setTimeout(() => {
            if (btn) btn.classList.remove('spin-anim');
